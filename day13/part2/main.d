module day13.part2.main;

import std;
import core.thread;
import core.stdc.stdlib : exit;

alias Tile = Tuple!(long, "x", long, "y", long, "id");
...
void main()
{
    ...
    memory.length = 4096;
    memory[0] = 2;
    auto programOutput = new Pipe!long();
    auto playOutput = new Pipe!long();

    Scheduler().schedule(createFiber(memory, playOutput, programOutput))
        .schedule(play(programOutput, playOutput)).run();

    playOutput.front.writeln;

}

auto play(Input, Output)(Input input, Output output)
{
    return new Fiber({
        long[Point] tiles;
        long score;
        ...
        while (true)
        {
            const data = input.chunks(3).map!((it) {
                immutable x = it.front;
                it.popFront();
                immutable y = it.front;
                it.popFront();
                immutable id = it.front;
                return Tile(x, y, id);
            }).array;

            tiles = data.filter!(it => !(it.x == -1 && it.y == 0))
                .fold!((t, it) { t[Point(it.x, it.y)] = it.id; return t; })(tiles);

            data.filter!(it => it.x == -1 && it.y == 0)
                .map!(it => it.id)
                .each!(it => score = it);

            if (!tiles.byValue.canFind(2))
                break;

...
void main()
{
    ...
    Scheduler().schedule(createFiber(memory, commands, responses))
        .schedule(findOxygen(responses, commands)).run();
}
...
auto findOxygen(Input, Output)(Input input, Output output)
{
    return new Fiber({
...
void print(long[long[2]] area, long[2] currentPosition)
{
    ...
    iota(maxY, minY - 1, -1).map!(i => iota(minX, maxX + 1).map!(j => [j,
                i] == currentPosition ? 'D' : [j, i] == [0, 0]
            ? 'S' : area.getOrElse([j, i], -1).predSwitch(-1, ' ', 0, '■', 1, '.', 2, 'O')))
        .each!writeln;
}

...
void main()
{
    ...
    auto area = memory.discoverArea;
    area.minutesTillFilledWithOxygen.writeln;
}

auto minutesTillFilledWithOxygen(long[long[2]] area)
{
    return recurrence!((areas, n) => areas[n - 1].spreadOxygen)(area).until!(
            it => !it.byValue.canFind(1))
        .tee!((it) { it.print; Thread.sleep(10.msecs); })
        .walkLength;
}

auto neighboursWithoutOxygen(long[long[2]] area, long[2] position)
{
    return [[1, 0], [0, 1], [-1, 0], [0, -1]].map!((it) {
        long[2] pos = it[] + position[];
        return pos;
    })
        .filter!(it => it in area && area[it] == 1);
}

auto spreadOxygen(long[long[2]] _area)
{
    auto area = _area.dup;
    area.dup.byPair().filter!(it => it.value == 2)
        .map!(it => area.neighboursWithoutOxygen(it.key))
        .joiner
        .each!(it => area[it] = 2);
    return area;
}

auto discoverArea(long[] memory)
{
    auto commands = new Pipe!long();
    auto responses = new Pipe!long();

    long[long[2]] area;

    auto scheduler = Scheduler();
    scheduler.schedule(createFiber(memory, commands, responses));
    scheduler.schedule(discover(responses, commands, scheduler, area));
    scheduler.run();
    return area;
}

long[2] rotate(long[2] input, int n)
{
    n = (n + 4) % 4;
    if (n == 0)
        return input;

    return [-input[1], input[0]].to!(long[2]).rotate(n - 1);
}

unittest
{
    assert([1, 0].rotate(1) == [0, 1]);
    assert([1, 0].rotate(-1) == [0, -1]);
}

long directionForMove(long[2] move)
{
    return move.predSwitch([0, 1], 1, [0, -1], 2, [-1, 0], 3, [1, 0], 4);
}

long surrounding(long[long[2]] area, long[2] currentPosition, long[2] lastMove, uint dir)
{
    long[2] pos = currentPosition[] + lastMove.rotate(dir)[];
    return area.getOrElse(pos, -1);
}

auto discover(Input, Output)(Input input, Output output, ref Scheduler scheduler,
        ref long[long[2]] outArea)
{
    return new Fiber({
        long[long[2]] area = [[0, 0]: 1];
        long[2] currentPosition = [0, 0];
        long[2] lastMove = [0, 1];
        long[2] lastNonWallMove = [0, 1];
        long counter;
        long allCounter;
        ...
        while (true)
        {
            foreach (i; [-1, 0, 1, 2])
            {
                if (!area.surrounding(currentPosition, lastNonWallMove, i) == 0)
                {
                    auto move = lastNonWallMove.rotate(i);
                    output.put(move.directionForMove);
                    lastMove = move;
                    break;
                }
            }
            ...
            Fiber.yield();
            auto status = input.front();
            input.popFront();
            long[2] newPos = currentPosition[] + lastMove[];
            if (newPos !in area && status)
            {
                counter++;
                allCounter++;
            }
            long* p = newPos in area;
            if (p && *p != 0)
            {
                counter--;
            }
            ...
            if (status)
            {
                ...
                lastNonWallMove = lastMove;
            }
            if (allCounter > 0 && currentPosition == [0, 0])
            {
                outArea = area;
                scheduler.terminate();
            }
...
void print(long[long[2]] area)
{
    auto minX = area.keys.map!(it => it[0]).minElement;
    auto minY = area.keys.map!(it => it[1]).minElement;
    auto maxX = area.keys.map!(it => it[0]).maxElement;
    auto maxY = area.keys.map!(it => it[1]).maxElement;

    iota(maxY, minY - 1, -1).map!(i => iota(minX, maxX + 1)
            .map!(j => area.getOrElse([j, i], -1).predSwitch(-1, ' ', 0, '■', 1, '.', 2, 'O')))
        .each!writeln;
}

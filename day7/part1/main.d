...
void main()
{
    ...
    iota(5).permutations
        .map!(phaseSettings => phaseSettings.fold!((input, phaseSetting) {
                LastOutput or;
                auto ir = [phaseSetting, input];
                executeProgram!(ir, or)(memory.dup);
                return or.get;
            })(0))
        .fold!max
        .writeln;
...
class OutputInstruction(alias or) : Instruction
{
private:
    Parameter value;
...
public:
    ...
    override Continue execute(ref size_t instructionPointer, int[] memory)
    {
        or.put(value.derefIfNeeded(memory));
        instructionPointer += 2;
        return Continue.yes;
    }
}
...
Instruction nextInstruction(alias ir, alias or)(size_t instructionPointer, int[] memory)
{
    ...
    case Opcode.output:
        return new OutputInstruction!(or)(instructionPointer, memory);
...
void executeProgram(alias ir, alias or)(int[] memory)
{
    size_t instructionPointer = 0;
    while (true)
    {
        auto instruction = nextInstruction!(ir, or)(instructionPointer, memory);

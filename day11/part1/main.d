...
void main()
{
    ...
    Scheduler().schedule(createFiber(memory, sensorData, actions))
        .schedule(readSensorFiber(actions, sensorData)).run();
}
...
auto readSensorFiber(Pipe!long i, Pipe!long o)
{
    return new Fiber(() {
        ...
        while (true)
        {
            ...
            case 3:
                ...
                break;
            default:
                throw new Exception("Invalid direction: %s".format(currentDirection));
            }
        }
        colors.length.writeln;
    });
}

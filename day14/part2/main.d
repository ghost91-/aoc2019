...
auto calculateRequiredOre(Reaction[string] reactions, ulong amount,
        ...
        string material, ulong[string] storedMaterial)
{
    ...
    immutable numberOfReactionsNeeded = requiredAmount / reaction.amount + (
            requiredAmount % reaction.amount > 0 ? 1 : 0);

    immutable canProduce = reaction.inputs
        .map!(input => reactions.canProduceWithResources(
                input.amount * numberOfReactionsNeeded, input.material, storedMaterial))
        .fold!and(true);
